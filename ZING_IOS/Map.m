...
- (void)viewDidLoad {
    ...
    [titleButton setTitle:[[NSUserDefaults standardUserDefaults] stringForKey:@"username"] forState:UIControlStateNormal];
...
- (void)updateUserLocation
{
    ...
    for(int i=0; i<[self.user.pairs count]; i++){
        ...
        pin.udid = [[self.user.pairs objectAtIndex:i] udid];
        pin.appID = [[self.user.pairs objectAtIndex:i] appID];
        pin.coordinate = [[self.user.pairs objectAtIndex:i] coordinates];
        if(pin.coordinate.latitude==0. && pin.coordinate.longitude==0.)continue; //skip if (0.0, 0.0) or no update (just in case)
        pin.title = [[self.user.pairs objectAtIndex:i] username]; //stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        pin.name = [[self.user.pairs objectAtIndex:i] username];  //stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        pin.shouldZoom = YES;
        ...
        if(distanceMeters>1610.)  // if less than a mile
            if([[NSUserDefaults standardUserDefaults] boolForKey:@"units"])
                pin.subtitle = [NSString stringWithFormat:@"%.2f km", distanceMeters/1000.];
            else
                pin.subtitle = [NSString stringWithFormat:@"%.2f miles", distanceMiles];
            else if([[NSUserDefaults standardUserDefaults] boolForKey:@"units"])
                pin.subtitle = [NSString stringWithFormat:@"%.2f m", distanceMeters];
            else
                pin.subtitle = [NSString stringWithFormat:@"%@ ft", distanceFeetComma];
...
-(void)mapView:(MKMapView *)mapView annotationView:(MKAnnotationView *)view calloutAccessoryControlTapped:(UIControl *)control {
    ...
    if ([annotation isKindOfClass:[Pin class]]){
        ...
        appIDToDelete = ((Pin *)annotation).appID;
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    else if([title isEqualToString:@"YES"])
    {
        ...
        [[GlobalData shared] sendNotification:[NSString stringWithFormat:@"%@ has unpaired you", self.user.username] to:appIDToDelete  ofType:@"RemovePair"];
        
        //]: self.user.username withAppID: appIDToDelete withMessage:[NSString stringWithFormat:@"%@ has unpaired you", self.user.username] andIdentification:@"RemovePair"];
        [self refreshMap];
    }
}
...
- (void)handlePushNotification:(NSDictionary *)notification withType:(NSString *)type {
    ...
    if([type isEqualToString:@"RemovePair"]){
        title = @"Removed Pair";
        alert = [[UIAlertView alloc] initWithTitle:title message:theMessage delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        alert.tag = 101;
        [alert show];
    } else if([type isEqualToString:@"CreatePair"]){
        title = @"Create Pair";
        NSLog(@"%@",title);
        alert = [[UIAlertView alloc] initWithTitle:title message:theMessage delegate:nil cancelButtonTitle:@"NO" otherButtonTitles:nil];
        [alert addButtonWithTitle:@"YES"];
        alert.tag = 102;
        [alert  show];
    } else if([type isEqualToString:@"AcceptPair"]){
        title = @"Accept Pair";
        alert = [[UIAlertView alloc] initWithTitle:title message:theMessage delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];;
        alert.tag = 103;
        [alert  show];
    } else if([type isEqualToString:@"RejectedPair"]){
        title = @"Rejected Pair";
        alert = [[UIAlertView alloc] initWithTitle:title message:theMessage delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];;
        alert.tag = 104;
        [alert  show];
...
{
    if (alertView.tag         == 101) { //RemovePair
        if (buttonIndex == 1){  //Clicked OK
            ;
        }
    } else  if (alertView.tag == 102) { //CreatePair
        if (buttonIndex == 1) {  //Clicked OK
            ;
        }
    } else  if (alertView.tag == 103) { //AcceptPair
        if (buttonIndex == 1) {  //Clicked NO
            ;
        }
        if (buttonIndex == 2) {  //Clicked YES
            ;
        }
    } else  if (alertView.tag == 104) { //RejectedPair
        if (buttonIndex == 1) {  //Clicked OK
            ;
        }
    } else  if (alertView.tag == 105) { //Message
        if (buttonIndex == 1) {  //Clicked OK
            ;
        }
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"Settings"]) {
        ...
        Settings *settings = [navigationController viewControllers][0];
